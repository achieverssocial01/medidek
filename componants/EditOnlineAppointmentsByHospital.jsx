...
const EditOnlineAppointmentsByHospital = () => {
  const [dates, setDates] = useState([]);
  const {values2,setvalues2,doctorDetail,selecteddate, setselecteddate} =useContext(UserContext)
  ...
  const [isfocus, setisfocusg] = useState(false);
   ...
   const  [data,setdata] =useState(false);
    ...
    const getdatewisedata =async()=>{
      const res = await axiosClient.get(`v2/getSlotDetailForDoctorForPerticularDate/${doctorDetail?._id}/${selecteddate?.currentDate}`);
    ...
    useEffect(() => {
      ...
      const start = new Date(`2023-01-01T${values2?.Starttime1}:00`);
      ...
      let currentTime = new Date(`2023-01-01T${values2?.Starttime1}:00`);
    ...
    }, [slotDuration, values2?.Starttime1]);
    ...
    useEffect(() => {
      const start = new Date(`2023-01-01T${values2?.Starttime3}:00`);
      start.setMinutes(start.getMinutes() + slotDuration);
      const generatedEndTimes3 = [];
      let currentTime = new Date(`2023-01-01T${values2?.Starttime3}:00`);
      while (currentTime < new Date(`2023-01-01T11:59:00`)) {
        currentTime.setMinutes(currentTime.getMinutes() + slotDuration);
    ...
    useEffect(() => {
      const start = new Date(`2023-01-01T${values2?.Starttime2}:00`);
      start.setMinutes(start.getMinutes() + slotDuration); 
      const generatedEndTimes2 = [];
      let currentTime = new Date(`2023-01-01T${values2?.Starttime2}:00`);
      while (currentTime < new Date(`2023-01-01T11:59:00`)) {
        currentTime.setMinutes(currentTime.getMinutes() + slotDuration);
    ...
    useEffect(() => {
      const start = new Date(`2023-01-01T${values2?.Endtime2}:00`);
      start.setMinutes(start.getMinutes() + slotDuration);

      const genratedStartTimes3 = [];
      let currentTime = new Date(`2023-01-01T${values2?.Endtime2}:00`);
      while (currentTime < new Date(`2023-01-01T11:59:00`)) {
        currentTime.setMinutes(currentTime.getMinutes() + slotDuration);
    ...
    useEffect(() => {
      const start = new Date(`2023-01-01T${values2?.Endtime1}:00`);
      start.setMinutes(start.getMinutes() + slotDuration);
      const genratedStartTimes2 = [];
      let currentTime = new Date(`2023-01-01T${values2?.Endtime1}:00`);
      while (currentTime < new Date(`2023-01-01T11:59:00`)) {
        currentTime.setMinutes(currentTime.getMinutes() + slotDuration);
    ...
    const saved = async() => {
      const res = await axiosClient.post("/v2/creatTokenForDoctor",{
        Starttime1:values2?.Starttime1,
        Endtime1:values2?.Endtime1,
        Starttime2:values2?.Starttime2,
        Endtime2:values2?.Endtime2,
        Starttime3:values2?.Starttime3,
        Endtime3:values2?.Endtime3,
        isholiday:false,
...
<ScrollView automaticallyAdjustKeyboardInsets={true}>
    ...
    style={styles.dropdown}
       ...
       onChange={item => setvalues2({...values2,Starttime1:item})}
       placeholder={values2?.Starttime1 ? values2?.Starttime1 :field}
       value={values2?.Starttime1}
       maxHeight={300}
...
<Text style={{fontWeight:"700",fontSize:15,padding:5}}>Choose endTime</Text>
    ...
    style={styles.dropdown}
       ...
       inputSearchStyle={styles.inputSearchStyle}
       searchPlaceholder="Search..."
       onChange={item => setvalues2({...values2,Endtime1:item})}
       placeholder={values2?.Endtime1 ? values2?.Endtime1 :field}
       value={values2?.Endtime1}
       disable= {save ? true :false}
       maxHeight={300}
       onFocus={() => setisfocusg(true)}
       onBlur={() => setisfocusg(true)}
...
<Text style={{fontWeight:"700",fontSize:15,padding:5}}>Choose startTime 2</Text>
    ...
    disable= {save ? true :false}
       ...
       inputSearchStyle={styles.inputSearchStyle}
       searchPlaceholder="Search..."
       onChange={item => setvalues2({...values2,Starttime2:item})}
       placeholder={values2?.Starttime2 ? values2?.Starttime2 :field}
       value={values2?.Starttime2}
       maxHeight={300}
       onFocus={() => setisfocusg(true)}
       onBlur={() => setisfocusg(true)}
...
<Text style={{fontWeight:"700",fontSize:15,padding:5}}>Choose endTime 2</Text>
    ...
    disable= {save ? true :false}
       ...
       inputSearchStyle={styles.inputSearchStyle}
       searchPlaceholder="Search..."
       onChange={item => setvalues2({...values2,Endtime2:item})}
       placeholder={values2?.Endtime2 ? values2?.Endtime2 :field}
       value={values2?.Endtime2}
       maxHeight={300}
       onFocus={() => setisfocusg(true)}
       onBlur={() => setisfocusg(true)}
...
<Text style={{fontWeight:"700",fontSize:15,padding:5}}>Choose startTime 3</Text>
    ...
    <Dropdown data={startTimes3}
     ...
     disable= {save ? true :false}
        ...
        inputSearchStyle={styles.inputSearchStyle}
        searchPlaceholder="Search..."
        onChange={item => setvalues2({...values2,Starttime3:item})}
        placeholder={values2?.Starttime3 ? values2?.Starttime3 :field}
        value={ values2?.Starttime3}
        maxHeight={300}
        onFocus={() => setisfocusg(true)}
        onBlur={() => setisfocusg(true)}
    />
...
<Text style={{fontWeight:"700",fontSize:15,padding:5}}>Choose endTime 3</Text>
    ...
    renderItem={item => <Timing item={item} />}
     ...
     disable= {save ? true :false}
        ...
        inputSearchStyle={styles.inputSearchStyle}
        searchPlaceholder="Search..."
        onChange={item => setvalues2({...values2,Endtime3:item})}
        placeholder={values2?.Endtime3 ? values2?.Endtime3 :field}
        value={ values2?.Endtime3}
        maxHeight={300}
        onFocus={() => setisfocusg(true)}
        onBlur={() => setisfocusg(true)}
    />
...
export default EditOnlineAppointmentsByHospital
