import { Button, StyleSheet, Text, View,FlatList,TouchableOpacity, KeyboardAvoidingView,ScrollView,Dimensions,ActivityIndicator,Alert } from 'react-native'
...
const DropDown = () => {
  ...
  const {userLogIn,values,setvalues,selecteddate, setselecteddate} =useContext(UserContext)
  ...
  const [time, settime] = useState('');
   ...
   const  [data,setdata] =useState(false);
    ...
    const getdatewisedata =async()=>{
      const res = await axiosClient.get(`v2/getAppointmentByTokenSlotDetailForDoctorForPerticularDate/${userLogIn?.user?._id}/${selecteddate?.currentDate}`);
    ...
    useEffect(()=>{
      getdatewisedata()
    },[selecteddate?.currentDate,reload])
